...
class Steam {
	 ...
	 */
	public static function downloadLeaderboardScore(id:String, downloadType:LeaderboardDownloadType = LeaderboardDownloadType.AroundUser, numBefore:Int = 0,
			numAfter:Int = 0):Bool {
		if (!active)
			return false;
		...
		if (startProcessingNow)
			processNextLeaderboardOp();
		return true;
	}

	private static function findLeaderboardIfNecessary(id:String) {
		if (!Lambda.has(leaderboardIds, id) && !Lambda.exists(leaderboardOps, function(op) {
			return Type.enumEq(op, FIND(id));
		})) {
	...
	public static function indicateAchievementProgress(id:String, curProgress:Int, maxProgress:Int):Bool {
		return active
			&& report("indicateAchivevementProgress", [id, Std.string(curProgress), Std.string(maxProgress)],
				SteamWrap_IndicateAchievementProgress(id, curProgress, maxProgress));
	}
	...
	private static function processNextLeaderboardOp() {
		...
		switch (op) {
			...
			case DOWNLOAD(id, downloadType, numBefore, numAfter):
				if (!report("Leaderboard.DOWNLOAD", [id, Std.string(downloadType), Std.string(numBefore), Std.string(numAfter)],
					SteamWrap_DownloadScores(id, downloadType, numBefore, numAfter)))
					processNextLeaderboardOp();
		}
	}
	...
	private static function steamWrap_onEvent(e:Dynamic) {
		...
		switch (type) {
			...
			case "UserAchievementStored":
				if (whenAchievementStored != null)
					whenAchievementStored(data);
			...
			case "ScoreUploaded":
				if (success) {
					...
					if (score != null && whenLeaderboardScoreUploaded != null)
						whenLeaderboardScoreUploaded(score);
				}
				processNextLeaderboardOp();
			case "UGCItemCreated":
			...
			case "LobbyCreated":
				if (matchmaking.whenLobbyCreated != null)
					matchmaking.whenLobbyCreated(success);
			case "LobbyJoined":
...
class EnumerateUserSubscribedFilesResult extends EnumerateUserPublishedFilesResult {
	...
	public function new(Result:EResult = 1, ResultsReturned:Int = 0, TotalResults:Int = 0, PublishedFileIds:Array<String> = null,
			TimeSubscribed:Array<Int> = null) {
	...
	public override function toString():String {
		return "EnumerateUserSubscribedFilesResult{result:" + result + ",resultsReturned:" + resultsReturned + ",totalResults:" + totalResults
			+ ",publishedFileIds:" + publishedFileIds + "timeSubscribed:" + timeSubscribed + "}";
	}
}
...
class EnumerateWorkshopFilesResult extends EnumerateUserPublishedFilesResult {
	...
	public function new(Result:EResult = 1, ResultsReturned:Int = 0, TotalResults:Int = 0, PublishedFileIds:Array<String> = null, Score:Array<Float> = null,
			AppID:Int = 0, StartIndex:Int = 0) {
		super(Result, ResultsReturned, TotalResults, PublishedFileIds);
		score = Score;
		appID = AppID;
	...
	public static function fromString(str:String):EnumerateWorkshopFilesResult {
		...
		for (str in arr) {
			if (str.indexOf(":") != -1) {
				...
				switch (nameValue[0]) {
					case "appID":
						appID = Util.str2Int(nameValue[1]);
					case "startIndex":
						startIndex = Util.str2Int(nameValue[1]);
					default: // do nothing
				}
			}
		}
	...
	public override function toString():String {
		return "EnumerateWorkshopFilesResult{result:" + result + ",resultsReturned:" + resultsReturned + ",totalResults:" + totalResults + ",appID:" + appID
			+ ",startIndex:" + startIndex + ",publishedFileIds:" + publishedFileIds + "}";
	}
}
...
class EnumerateUserPublishedFilesResult {
	...
	public static function fromString(str:String, ?resultObject:EnumerateUserPublishedFilesResult):EnumerateUserPublishedFilesResult {
		...
		for (str in arr) {
			if (str.indexOf(":") != -1) {
				...
				switch (nameValue[0]) {
					case "result":
						result = Util.str2Int(nameValue[1]);
					case "resultsReturned":
						resultsReturned = Util.str2Int(nameValue[1]);
					case "totalResults":
						totalResults = Util.str2Int(nameValue[1]);
					case "publishedFileIds":
						publishedFileIds.push(nameValue[1]);
	...
	public function toString():String {
		return "EnumerateUserPublishedFilesResult{result:" + result + ",resultsReturned:" + resultsReturned + ",totalResults:" + totalResults
			+ ",publishedFileIds:" + publishedFileIds + "}";
	}
}
...
class DownloadUGCResult {
			...
			if (str.indexOf(":") != -1) {
				...
				switch (nameValue[0]) {
					case "result":
						result = Util.str2Int(nameValue[1]);
					case "fileHandle":
						fileHandle = nameValue[1];
					case "appID":
						appID = Util.str2Int(nameValue[1]);
					case "sizeInBytes":
						sizeInBytes = Util.str2Int(nameValue[1]);
					case "fileName":
						fileName = nameValue[1];
					case "steamIDOwner":
						steamIDOwner = nameValue[1];
				}
			}
		}
	...
	public function toString():String {
		return ("{result:" + result + ",fileHandle:" + fileHandle + ",appID:" + appID + ",sizeInBytes:" + sizeInBytes + ",fileName:" + fileName
			+ ",steamIDOwner:" + steamIDOwner + "}");
	}
}
...
class GetPublishedFileDetailsResult {
	...
	public function new(Result:EResult = Fail, FileID:String = "", CreatorAppID:String = "", ConsumerAppID:String = "", Title:String = "",
			Description:String = "", FileHandle:String = "", PreviewFileHandle:String = "", SteamIDOwner:String = "", TimeCreated:Int = 0,
			TimeUpdated:Int = 0, Visibility:EPublishedFileVisibility, Banned:Bool = false, Tags:String = "", TagsTruncated:Bool = false, FileName:String = "",
			FileSize:Int = 0, PreviewFileSize:Int = 0, URL:String = "", FileType:EWorkshopFileType, AcceptedForUse:Bool = false) {
		result = Result;
		fileID = FileID;
			...
			if (str.indexOf(":") != -1) {
				...
				switch (nameValue[0]) {
					...
					case "publishedFileID":
						fileID = nameValue[1];
					case "creatorAppID":
						creatorAppID = nameValue[1];
					case "consumerAppID":
						consumerAppID = nameValue[1];
					case "title":
						title = nameValue[1];
					case "description":
					...
					case "fileHandle":
						fileHandle = nameValue[1];
					case "previewFileHandle":
						previewFileHandle = nameValue[1];
					case "steamIDOwner":
						steamIDOwner = nameValue[1];
					case "timeCreated":
						timeCreated = Util.str2Int(nameValue[1]);
					case "timeUpdated":
						timeUpdated = Util.str2Int(nameValue[1]);
					case "visibility":
						visibility = Util.str2Int(nameValue[1]);
					case "banned":
						banned = Util.boolify(nameValue[1]);
					case "tagsTruncated":
						tagsTruncated = Util.boolify(nameValue[1]);
					case "fileName":
						fileName = nameValue[1];
					case "fileSize":
						fileSize = Util.str2Int(nameValue[1]);
					case "url":
						url = nameValue[1];
					case "fileType":
						fileType = Util.str2Int(nameValue[1]);
					case "acceptedForUse":
						acceptedForUse = Util.boolify(nameValue[1]);
				}
			}
		}

		return new GetPublishedFileDetailsResult(result, fileID, creatorAppID, consumerAppID, title, description, fileHandle, previewFileHandle, steamIDOwner,
			timeCreated, timeUpdated, visibility, banned, tagsTruncated, fileName, fileSize, url, fileType, acceptedForUse);
	}
}
...
class SteamUGCDetails {
	...
	public function new(PublishedFileId:String = "", Result:EResult = EResult.Fail, FileType:EWorkshopFileType = EWorkshopFileType.Community,
			CreatorAppID:String = "", ConsumerAppID:String = "", Title:String = "", Description:String = "", SteamIDOwner:String = "", TimeCreated:Float = 0,
			TimeUpdated:Float = 0, TimeAddedToUserList:Float = 0, Visibility:EPublishedFileVisibility = EPublishedFileVisibility.Private, Banned:Bool = false,
			AcceptedForUse:Bool = false, TagsTruncated:Bool = false, Tags:String = "", File:String = "", PreviewFile:String = "", FileName:String = "",
			FileSize:Int = 0, PreviewFileSize:Int = 0, URL:String = "", VotesUp:Int = 0, VotesDown:Int = 0, Score:Float = 0.0, NumChildren:Int = 0) {
			...
			if (str.indexOf(":") != -1) {
				...
				switch (nameValue[0]) {
					case "publishedFileId":
						PublishedFileId = val;
					case "result":
						Result = Util.str2Int(val);
					case "fileType":
						FileType = Util.str2Int(val);
					case "creatorAppID":
						CreatorAppID = val;
					case "consumerAppID":
						ConsumerAppID = val;
					...
					case "steamIDOwner":
						SteamIDOwner = val;
					case "timeCreated":
						TimeCreated = Util.str2Float(val);
					case "timeUpdate":
						TimeUpdated = Util.str2Float(val);
					case "timeAddedToUserList":
						TimeAddedToUserList = Util.str2Float(val);
					case "visibility":
						Visibility = Util.str2Int(val);
					case "banned":
						Banned = Util.boolify(val);
					case "acceptedForUse":
						AcceptedForUse = Util.boolify(val);
					case "tagsTruncated":
						TagsTruncated = Util.boolify(val);
					case "tags":
						Tags = val;
					case "file":
						File = val;
					...
					case "fileName":
						FileName = val;
					case "fileSize":
						FileSize = Util.str2Int(val);
					case "previewFileSize":
						PreviewFileSize = Util.str2Int(val);
					case "url":
						URL = val;
					case "votesUp":
						VotesUp = Util.str2Int(val);
					case "votesDown":
						VotesDown = Util.str2Int(val);
					case "score":
						Score = Util.str2Float(val);
					case "numChildren":
						NumChildren = Util.str2Int(val);
				}
			}
		}

		return new SteamUGCDetails(PublishedFileId, Result, FileType, CreatorAppID, ConsumerAppID, Title, Description, SteamIDOwner, TimeCreated, TimeUpdated,
			TimeAddedToUserList, Visibility, Banned, AcceptedForUse, TagsTruncated, Tags, File, PreviewFile, FileName, FileSize, PreviewFileSize, URL,
			VotesUp, VotesDown, Score, NumChildren);
	}
